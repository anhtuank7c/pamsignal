...
| **Ubuntu** | 20.04 LTS (Focal) | 245 | 8.2p1 | ✅ Compiles + runs; **continuously CI-tested**; per-release Focal `.deb` attached to the GitHub release | ESM-only since 2025-04-30 — only Ubuntu Pro subscribers get host-level security updates. No gh-pages apt pocket (the per-distroseries repo infrastructure only pays off above ~30 hosts); download `pamsignal_*_focal_amd64.deb` directly from the [GitHub release page](https://github.com/anhtuank7c/pamsignal/releases) and install with `apt install ./<file>.deb`. Two systemd directives (`ProtectProc=invisible`, `ProcSubset=pid`, both added in systemd v247) are logged-and-ignored at unit load — systemd is forward-compatible, the daemon still runs; the live security score is ~1-2 points higher than on 22.04+ but well within the "OK" band. Plan a migration to 22.04 LTS (Standard Support until April 2027) before April 2030 when Focal exits ESM. |
...
**Ubuntu 20.04 (Focal) is the exception within Tier 2.** It is continuously tested in CI (the `build-deb-focal` and `test-deb-focal` jobs in `.github/workflows/release-packages.yml` build and smoke-test a Focal-targeted `.deb` against an `ubuntu:20.04` container every release), and the resulting `pamsignal_*_focal_amd64.deb` is attached to each GitHub release. Operators install via direct download rather than apt repo. This treatment is reserved for Focal specifically because of the ESM lifecycle math — supporting an actively-tested release-artifact build is cheap, while running a full apt pocket for a frozen, ESM-only distro is not.
...
| **Debian** | 11 (Bullseye) | 247 | 2.31 | Sandbox posture comparable to Ubuntu 20.04 (missing `ProtectProc`/`ProcSubset`) — but Bullseye is not currently in the CI matrix and no Bullseye-targeted `.deb` is shipped, so we don't make a support claim. If you want to run pamsignal on Bullseye, build from source via `dpkg-buildpackage` on a Bullseye host — it should work, but you're on your own for regressions. |
