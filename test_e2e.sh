...
echo "[1/5] Stopping existing pamsignal processes..."
sudo pkill pamsignal 2>/dev/null && sleep 1 || true

# Copy fresh binary
echo "[2/5] Copying fresh binary to /tmp..."
sudo cp ./build/pamsignal /tmp/pamsignal

# Start as daemon
echo "[3/5] Starting pamsignal daemon..."
...
# Check startup logs
echo "[4/5] Startup logs:"
journalctl -t pamsignal --since "10 sec ago" --no-pager
echo ""

# Interactive tests
echo "[5/5] Now run these tests manually:"
echo ""
