...
int ps_pidfile_acquire(void) {
    pidfile_fd =
        open(PS_PID_FILE, O_WRONLY | O_CREAT | O_NOFOLLOW | O_EXCL, 0600);
    if (pidfile_fd < 0) {
        // If file exists from a previous unclean shutdown, unlink and retry
        // once
        if (errno == EEXIST) {
            unlink(PS_PID_FILE);
            pidfile_fd = open(PS_PID_FILE,
                              O_WRONLY | O_CREAT | O_NOFOLLOW | O_EXCL, 0600);
        }
        if (pidfile_fd < 0)
            return PS_ERR_INIT;
    }
