...
- [x] **Issue templates** (`.github/ISSUE_TEMPLATE/`). Modern YAML form-based: `bug_report.yml` requires version + distro + install method + reproduction steps + expected/actual + redacted journal output + redacted config; `feature_request.yml` requires a use-case statement + alternatives-considered + threat-model alignment ("which attack does this strengthen?") so the discussion starts at the design level rather than the implementation level. `config.yml` disables blank issues, points security reports at GitHub Security Advisories, and routes "question" traffic to existing issues with the `question` label.
